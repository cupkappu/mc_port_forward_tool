...
public final class McTransportCommands {
    ...
    private static ResolvedPlayer resolvePlayer(MinecraftServer server, String playerName) {
        ...
        Optional<GameProfile> cached = server.getUserCache() == null
                ? Optional.empty()
                : server.getUserCache().findByName(playerName);
        if (cached.isPresent()) {
            GameProfile profile = cached.get();
            return new ResolvedPlayer(profile.getId(), profile.getName());
        }
        throw new IllegalArgumentException("Cannot resolve player UUID for " + playerName);
    }
